...
# Build
xcodebuild -project VoKey.xcodeproj -scheme VoKey -configuration Debug build

# Run all tests (99 tests)
xcodebuild test -project VoKey.xcodeproj -scheme VoKey -configuration Debug

# Run a single test class
xcodebuild test -project VoKey.xcodeproj -scheme VoKey -only-testing:VoKeyTests/AppStateTests

# Run a single test method
xcodebuild test -project VoKey.xcodeproj -scheme VoKey -only-testing:VoKeyTests/AppStateTests/test26_initialState_isIdle

# Launch built app
open ~/Library/Developer/Xcode/DerivedData/VoKey-*/Build/Products/Debug/VoKey.app
```
...
VoKey is a macOS menu bar app for global voice input. User presses Option+Space (configurable), speaks, presses again to stop — recognized text is inserted at the cursor position in any app via CGEvent keyboard simulation (no clipboard).
...
**Protocol-based DI**: All services conform to protocols in `Services/Protocols.swift`. Production types are injected in `VoKeyApp.swift`; tests use mocks from `VoKeyTests/Mocks/TestMocks.swift`.
...
- `HistoryStore` — SQLite3 C API, stores all recognition results in `~/Library/Application Support/VoKey/history.db`
...
- **HotkeyConfig UserDefaults**: Hotkey settings stored in UserDefaults. If hotkey stops working, check `defaults read com.vokey.app` for corrupted values.

## Test Organization

Tests are in `VoKeyTests/` organized by layer:
...
Mocks: `VoKeyTests/Mocks/TestMocks.swift` has `MockSpeechRecognizer`, `MockAudioRecorder`, `MockPermissionChecker`, `MockPunctuationService`, `MockAudioBackupService`.

## Project Configuration

- **Build tool**: XcodeGen (`project.yml`)
- **Bundle ID**: `com.vokey.app`
...
- **Bridging header**: `VoKey/SherpaOnnx/SherpaOnnx-Bridging-Header.h`
