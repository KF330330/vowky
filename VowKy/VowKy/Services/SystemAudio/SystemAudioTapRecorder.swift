...
final class SystemAudioTapRecorder: AudioRecorderProtocol, SystemAudioSilenceReporting {
    ...
    private func handleInput(
        ...
        for layout in plan.layouts {
            ...
            // 声道数以 buffer 自报的 mNumberChannels 为准,不信流的 VirtualFormat——
            // 实测(macOS 26.5)VirtualFormat 会报成输出设备的 6 声道布局,真实 buffer 却是 tap 的单声道;
            // 按 VirtualFormat 校验会把每个周期都判成畸形、一个样本都收不到。
            if layout.isNonInterleaved {
                ...
                for offset in 0..<layout.bufferCount {
                    ...
                    guard let samples = Self.floatSamples(of: buffer, bytesPerFrame: UInt32(MemoryLayout<Float>.size)) else {
                        logMalformedPeriod("stream \(layout.streamID) buffer \(offset) size \(buffer.mDataByteSize) not a multiple of 4")
                        return
                    }
                ...
                let frameWidth = UInt32(channels * MemoryLayout<Float>.size)
                guard let data = buffer.mData, buffer.mDataByteSize % frameWidth == 0 else {
                    logMalformedPeriod("stream \(layout.streamID) buffer shape ch=\(buffer.mNumberChannels) bytes=\(buffer.mDataByteSize) not a multiple of frame width \(frameWidth)")
                    return
                }
                let frames = Int(buffer.mDataByteSize / frameWidth)
    ...
    private static func buildStreamPlan(
        ...
        if tapIndices.isEmpty {
            ...
            // 多个候选时才用 tap 自身格式做甄别;只有一个候选就直接采信位置推导——
            // 未获系统录音授权时聚合会给出「输出设备声道布局」的占位流(实测 6ch,tap 自身是 1ch),
            // 按格式硬筛会把这种情况变成启动失败,而它应该走「录到全零 → 静音告警」那条路。
            if derived.count > 1, tapFormatStatus == noErr {
                let matching = derived.filter {
                    formats[$0].mChannelsPerFrame == tapFormat.mChannelsPerFrame
                        && formats[$0].mSampleRate == tapFormat.mSampleRate
                }
                if !matching.isEmpty { derived = matching }
            }
        ...
        // tap 数据的权威格式取 kAudioTapPropertyFormat(tap 自己报的)。聚合流的 VirtualFormat
        // 在未授权时会谎报成输出设备的声道布局(实测 6ch),只有 tap 自报的格式与真实 buffer 一致。
        let dataFormat = tapFormatStatus == noErr ? tapFormat : formats[tapIndex]
        guard (dataFormat.mFormatFlags & kAudioFormatFlagIsFloat) != 0,
              dataFormat.mBitsPerChannel == 32,
              dataFormat.mSampleRate > 0,
              dataFormat.mChannelsPerFrame > 0 else {
            NSLog("[VowKy][SysAudio] unsupported tap format (expect Float32): flags=\(dataFormat.mFormatFlags) bits=\(dataFormat.mBitsPerChannel) ch=\(dataFormat.mChannelsPerFrame) rate=\(dataFormat.mSampleRate)")
            throw SystemAudioRecorderError.converterCreationFailed
        }
        if tapFormatStatus == noErr, formats[tapIndex].mChannelsPerFrame != tapFormat.mChannelsPerFrame {
            // 实测(macOS 26.5,2026-08-25 标定):聚合流的 VirtualFormat 会报成输出设备的声道布局
            // (内置扬声器 6ch),而真实 buffer 是 tap 自报的单声道。以 tap 格式为准即可正常采集。
            NSLog("[VowKy][SysAudio] aggregate stream VirtualFormat says \(formats[tapIndex].mChannelsPerFrame) ch, tap says \(tapFormat.mChannelsPerFrame) ch — trusting tap format")
        }

        // buffer 分组:交错流 1 buffer,非交错流每声道 1 buffer。tap 流按其自报格式分组,
        // 其余输入流(双工设备自带的)按各自 VirtualFormat 分组,只用于推进 buffer 游标。
        let layouts = streams.indices.map { index -> StreamLayout in
            let isTap = index == tapIndex
            let format = isTap ? dataFormat : formats[index]
            return StreamLayout(
        ...
        return StreamPlan(layouts: layouts, sampleRate: dataFormat.mSampleRate)
    }
